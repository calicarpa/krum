"""MoNNA vs Mean on CIFAR-10, reproducing Table 2 (Appendix D.2) of the ICML 2023 paper.

Uses the paper's own CNN (``Monna2023CNNCifar10``), learning rate, batch size,
momentum, weight decay, worker/Byzantine counts, round count, and data
heterogeneity (``alpha = 5``), comparing ``f = 0`` (baseline) against the two
attacks this library implements an exact match for — ALIE ("little" in the
paper) and sign-flip ("signflipping") — under MoNNA's nearest-neighbor
averaging and a plain (non-robust) mean.
...
from krum.primitives.models.cnn import Monna2023CNNCifar10

from .run import monna_experiment

# --- Configurable parameters (Table 2, Appendix D.2 — CIFAR-10 column) ---
DATASET = "cifar10"
DATA_DIR = "data"
ROUNDS = 2000  # T
EVAL_EVERY = 50
...
LEARNING_RATE = 0.5  # gamma
BETA = 0.99
WEIGHT_DECAY = 1e-2  # l2-regularization
PARTITION = "dirichlet"
ALPHA_VALUES = [5]  # data heterogeneity, single value tested in the paper
SEED = 0
NUM_WORKERS = 0
...
def main() -> None:
    """Run the CIFAR-10 paper-reproduction experiment."""
    orchestrator = Orchestrator("monna_icml_2023_experiment_cifar")
