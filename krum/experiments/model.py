...
class Model:
        ...
        def add_custom_models(name, module, _):
            ...
            for model in exports:
                ...
                if fullname in cls.__models:
                    tools.warning(
                        f"Unable to make available model {model!r} from module {name!r}, as the name {fullname!r} already exists"
                    )
                    continue
    ...
    def __init__(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            if not isinstance(model, torch.nn.Module):
                raise tools.UserException(
                    f"Expected built model {name!r} to be an instance of 'torch.nn.Module', found {getattr(type(model), '__name__', '<unknown>')!r} instead"
                )
        ...
        params = tools.flatten(
            model.parameters()
        )  # NOTE: Ordering across runs/nodes seems to be ensured (i.e. only dependent on the model constructor)
        # Finalization
        self._model = model
        self._name = name
        self._config = config
        self._params = params
        self._gradient = None
        self._defaults = {"trainset": None, "testset": None, "loss": None, "criterion": None, "optimizer": None}
