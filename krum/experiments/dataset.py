...
transforms_horizontalflip = [torchvision.transforms.RandomHorizontalFlip(), torchvision.transforms.ToTensor()]
transforms_mnist = [
...
class Dataset:
    ...
    def get_default_root(cls):
        ...
        if not cls.__default_root.exists():
            ...
            tools.warning(
                f"Default dataset root {str(cls.__default_root)!r} does not exist, falling back to local temporary directory {tmpdir!r}",
                context="experiments",
            )
                ...
                def make_builder(constructor, name):
                    def builder(root, batch_size=None, shuffle=False, num_workers=1, *args, **kwargs):
                        ...
                        assert isinstance(data, torch.utils.data.Dataset), (
                            f"Internal heuristic failed: {name!r} was not a dataset name"
                        )
        ...
        def add_custom_datasets(name, module, _):
            ...
            for dataset in exports:
                ...
                if fullname in cls.__datasets:
                    tools.warning(
                        f"Unable to make available dataset {dataset!r} from module {name!r}, as the name {fullname!r} already exists"
                    )
                    continue
    ...
    def epoch(self, config=None):
        ...
        assert isinstance(self._loader, torch.utils.data.DataLoader), (
            "Full epoch iteration only possible for PyTorch's DataLoader-based datasets"
        )
        ...
        def generator():
            ...
            try:
                while True:
                    ...
                    if config is not None:
                        tns = type(tns)(
                            tn.to(device=config["device"], non_blocking=config["non_blocking"]) for tn in tns
                        )
                    yield tns
...
def batch_dataset(inputs, labels, train=False, batch_size=None, split=0.75):
    ...
    if dataset_len < 1 or len(labels) != dataset_len:
        raise RuntimeError(
            f"Invalid or different input/output tensor lengths, got {len(inputs)} for inputs, got {len(labels)} for labels"
        )
