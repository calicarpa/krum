...
>>> trainset, testset = make_datasets("cifar10", train_batch=128, test_batch=256)
        ...
        if isinstance(data, str):
            ...
            if build is None:
                raise tools.UnavailableException(
                    datasets, name, what="dataset name"
                )
...
def make_datasets(
    ...
    **custom_args : object
    ...
    if isinstance(num_workers, int):
        ...
        assert (
            isinstance(num_workers, tuple) and len(num_workers) == 2
        ), num_workers_errmsg
        train_workers, test_workers = num_workers
...
def batch_dataset(
    inputs, labels, train=False, batch_size=None, split=0.75
):
    """
    ...
    def test_gen(inputs, labels, batch):
        ...
        while True:
            ...
            if end > datalen:
                select = list(range(cursor, datalen)) + list(
                    range(end % datalen)
                )
                yield inputs[select], labels[select]
            else:
