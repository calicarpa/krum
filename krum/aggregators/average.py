...
def _influence(honests: list[torch.Tensor], attacks: list[torch.Tensor], **kwargs) -> float:
...
class Average(Aggregator):
    ...
    spec = AggregatorSpec(
        name="average",
        aliases=("mean", "Mean", "Average"),
        description="Simple arithmetic mean of all submitted gradients.",
    ...
    def influence(honests: list[torch.Tensor], attacks: list[torch.Tensor], **kwargs) -> float:
        ...
        **kwargs : object
        ...
        """
        return _influence(honests=honests, attacks=attacks, **kwargs)
...
def aggregate(gradients: list[torch.Tensor], **kwargs) -> torch.Tensor:
    ...
    **kwargs : object
    ...
    The output tensor is a new tensor, not aliasing any input tensor.
    """
    return Average().aggregate(gradients=gradients, **kwargs)
...
def check(gradients: list[torch.Tensor], **kwargs) -> str | None:
    ...
    **kwargs : object
    ...
    str or None
        ...
        message.
    """
    return Average().check(gradients=gradients, **kwargs)
...
def influence(honests: list[torch.Tensor], attacks: list[torch.Tensor], **kwargs) -> float:
    ...
    **kwargs : object
    ...
    float
        Ratio of Byzantine gradients in the aggregation (attackers / total).
    """
    return Average.influence(honests=honests, attacks=attacks, **kwargs)
