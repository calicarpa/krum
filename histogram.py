...
try:
    ...
    def gtk_run(closure):
        ...
        """
        tools.warning(f"GTK 3.0 is unavailable: {_gtk_err}")
...
class LinePlot:
    ...
    def _get_line_style(cls, ln):
        ...
        """
        return cls.linestyles[ln % len(cls.linestyles)], f"C{ln}"
    ...
    def include(self, data, *cols, errs=None, lalp=1.0, ccnt=None):
    ...
    def include_single(self, data, key, col, err=None, lalp=1.0, ccnt=None):
    ...
    def close(self):
        ...
        ):  # The documentation of 'plt.close' does not explicitly specify that multiple calls are allowed on the same 'Figure'
            plt.close(self._fig)
            self._fig = None
