...
class Loss:
    ...
    def __mul__(self, factor):
        ...
        return type(self)(
            type(self).__reserved_init, mul, factor * (1.0 if self._fact is None else self._fact), self._name
        )
...
class Criterion:
    ...
    class _TopkCriterion:
        ...
        def __call__(self, output, target):
            ...
            return torch.cat(
                (res.unsqueeze(0), torch.tensor(target.shape[0], dtype=res.dtype, device=res.device).unsqueeze(0))
            )
    ...
    def _get_criterions(cls):
        ...
        # Initialize the dictionary
        cls.__criterions = {"top-k": cls._TopkCriterion, "sigmoid": cls._SigmoidCriterion}
        # Return the dictionary
        return cls.__criterions
