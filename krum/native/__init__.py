...
def _build_and_load():
    ...
    if debug_mode_in_env:
        ...
        if value in ["0", "n", "no", "false"]:
            ...
            tools.fatal(
                "{!r} defined in the environment, but with unexpected soft-boolean {!r}".format(
                    debug_mode_envname, f"{debug_mode_envname}={raw}"
                )
            )
    else:
    ...
    if debug_mode:
        ...
        tools.warning(
            "Native modules compiled in debug mode; {}define {!r} in the environment or{} run python with -O/-OO options to compile in release mode".format(
                "re" if debug_mode_in_env else "",
                f"{debug_mode_envname}=1",
        ...
        if quiet_envname not in os.environ:
            ...
            tools.trace(
                "Native modules compiled in release mode; {}define {!r} in the environment or{} run python without -O/-OO options to compile in debug mode".format(
                    "re" if debug_mode_in_env else "",
                    f"{debug_mode_envname}=0",
                    " undefine it and" if debug_mode_in_env else "",
                )
            )
            tools.trace(f"Define {quiet_envname!r} in the environment to hide these messages in release mode")
            ...
            if ident in ident_to_is_python:
                ...
                if depsfile.exists():
                    for modname in depsfile.read_text().splitlines():
                        ...
                        if not res:  # Unable to build a dependency
                            if len(deps) == 0:
                                tools.warning(
...
from krum import tools as _tools
