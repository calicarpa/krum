"""Tests for the Medoid aggregator."""

import unittest

import torch

from krum.primitives.aggregators.medoid import Medoid
...
class MedoidTest(unittest.TestCase):
    ...
    def test_aggregate_multidimensional(self) -> None:
        """Medoid works on multidimensional gradients."""
    ...
    def test_aggregate_identical_gradients(self) -> None:
        """Medoid returns the first gradient when all are identical."""
        grads = torch.tensor([[1.0, 2.0], [1.0, 2.0], [1.0, 2.0]])
        result = Medoid.aggregate(grads, n=3, f=0)
        self.assertTrue(torch.equal(result, grads[0]))

    def test_aggregate_single_gradient(self) -> None:
        """Medoid with a single gradient returns it unchanged."""
        grads = torch.tensor([[7.0, 8.0, 9.0]])
        result = Medoid.aggregate(grads, n=1, f=0)
        self.assertTrue(torch.equal(result, grads[0]))

    def test_aggregate_ties_broken_by_smallest_index(self) -> None:
        """Medoid breaks ties by returning the gradient with the smallest index."""
        grads = torch.tensor([[0.0], [2.0], [2.0]])
        result = Medoid.aggregate(grads, n=3, f=0)
        self.assertTrue(torch.equal(result, grads[1]))

    def test_aggregate_f_is_ignored(self) -> None:
        """Medoid accepts any valid f but does not use it."""
        grads = torch.tensor([[0.0], [0.5], [1.0], [100.0]])
        result = Medoid.aggregate(grads, n=4, f=1)
        self.assertAlmostEqual(result.item(), 0.5)
    ...
    def test_aggregate_preserves_dtype(self) -> None:
        ...
        grads = torch.tensor([[0.0], [0.5], [1.0], [100.0]], dtype=torch.float64)
        result = Medoid.aggregate(grads, n=4, f=1)
