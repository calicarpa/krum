...
class Checkpoint:
    ...
    _transfers = {Model: (lambda x: x._model), Optimizer: (lambda x: x._optim)}
    ...
    def _prepare(cls, instance):
        ...
        if instance_cls in cls._transfers:
            res = cls._transfers[instance_cls](instance)
        else:
            res = instance
        ...
        return res, tools.fullqual(instance_cls)
    ...
    def restore(self, instance, nothrow=False):
        ...
        if key in self._store:
            ...
            if __debug__ and not self._copied[key]:
                tools.warning(
                    f"Restoring a state dictionary reference in an instance of {tools.fullqual(type(instance))}; the resulting behavior may not be the one expected"
                )
        elif not nothrow:
    ...
    def save(self, filepath, overwrite=False):
        ...
        if pathlib.Path(filepath).exists() and not overwrite:
            raise tools.UserException(
                f"Unable to save checkpoint in existing file {str(filepath)!r} (overwriting has not been allowed by the caller)"
            )
        # (Over)write the file
