...
class Checkpoint:
    ...
    ``load_state_dict`` protocol (e.g. ``torch.nn.Module``, ``torch.optim.Optimizer``).
    It also knows how to unwrap :class:`~experiments.model.Model` and
    ...
    def load(self, filepath, overwrite=False):
        ...
        if not overwrite and len(self._store) > 0:
            raise tools.UserException(
                "Unable to load into a non-empty checkpoint"
            )
        # Load the file
