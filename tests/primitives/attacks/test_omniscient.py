...
class OmniscientAttackTest(unittest.TestCase):
    ...
    def test_generate_returns_correct_shape(self) -> None:
        ...
        full = torch.ones(10)
        byz = OmniscientAttack.generate(honest, f=3, full_gradient=full)
    ...
    def test_generate_sends_negated_scaled_full_gradient(self) -> None:
        ...
        byz = OmniscientAttack.generate(honest, f=2, full_gradient=full, kappa=100.0)
        expected = -100.0 * full
    ...
    def test_generate_zero_byzantine(self) -> None:
        ...
        honest = torch.randn(5, 10)
        full = torch.ones(10)
        byz = OmniscientAttack.generate(honest, f=0, full_gradient=full)
    ...
    def test_rejects_negative_kappa(self) -> None:
        ...
        with self.assertRaises(ValueError):
            OmniscientAttack.generate(honest, f=3, full_gradient=full, kappa=-1.0)
    ...
    def test_rejects_negative_f(self) -> None:
        ...
        full = torch.ones(10)
        with self.assertRaises(ValueError):
            OmniscientAttack.generate(honest, f=-1, full_gradient=full)
    ...
    def test_rejects_2d_full_gradient(self) -> None:
        ...
        with self.assertRaises(ValueError):
            OmniscientAttack.generate(honest, f=3, full_gradient=full)

    def test_rejects_empty_honest_gradients(self) -> None:
        """Check raises ValueError when no honest gradients."""
        full = torch.ones(10)
        with self.assertRaises(ValueError):
            OmniscientAttack.generate(torch.empty((0, 5)), f=3, full_gradient=full)
    ...
    def test_accepts_sequence_of_per_worker_vectors(self) -> None:
        ...
        from_sequence = OmniscientAttack.generate(as_sequence, f=3, full_gradient=full)
        from_tensor = OmniscientAttack.generate(as_tensor, f=3, full_gradient=full)
    ...
    def test_writes_into_out_buffer_and_returns_it(self) -> None:
        ...
        result = OmniscientAttack.generate(honest, out, f=3, full_gradient=full)

        self.assertIs(result, out)
        self.assertTrue(torch.allclose(result, OmniscientAttack.generate(honest, f=3, full_gradient=full)))
