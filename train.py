...
with tools.Context("setup", "info"):
    ...
    if args.device_gar.lower() == "same":
        ...
        config_gar = experiments.Configuration(
            dtype=config["dtype"],
            device=(None if args.device_gar.lower() == "auto" else args.device_gar),
            noblock=config["non_blocking"],
        )
    # Defense
    ...
    trainset, testset = experiments.make_datasets(
        args.dataset,
        args.batch_size,
        ...
        try:
            ...
            try:
                ...
                result_make(
                    ...
                    "Average loss",
                    "l2 from origin",
                    "Honest gradient deviation",
                    "Attack gradient deviation",
                    "Honest gradient norm",
                    "Attack gradient norm",
                    "Defense gradient norm",
                    "Honest max coordinate",
                    "Attack max coordinate",
                    "Defense max coordinate",
                    "Honest-attack cosine",
                    "Honest-defense cosine",
                ...
                tools.warning(
                    f"Unable to create some result files in directory {str(resdir)!r} ({err}); some result(s) may be missing"
                )
...
with tools.Context("training", "info"):
    ...
    try:
        while not exit_is_requested():
            ...
            grad_attacks = attack.checked(
                grad_honests=grad_honests_gar,
                f_decl=args.nb_decl_byz,
                f_real=args.nb_real_byz,
                model=model,
                defense=defense,
                **args.attack_args,
            )
            ...
            if fd_study is not None:
                ...
                result_store(
                    ...
                    steps,
                    datapoints,
                    float_format % loss_avg,
                    float_format % l2_origin,
                    float_format % honest_norm_dev,
                    float_format % attack_norm_dev,
                    float_format % honest_norm_avg,
                    float_format % attack_norm_avg,
                    float_format % defense_norm_avg,
                    float_format % honest_norm_max,
                    float_format % attack_norm_max,
                    float_format % defense_norm_max,
                    float_format % cosin_honatt,
                    float_format % cosin_hondef,
                    float_format % cosin_attdef,
                )
            # ------------------------------------------------------------------------ #
